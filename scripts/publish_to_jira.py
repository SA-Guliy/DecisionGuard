...
def main() -> None:
    parser = argparse.ArgumentParser(description="Post-acceptance non-critical Jira publisher.")
    parser.add_argument("--run-id", required=True)
    args = parser.parse_args()

    jira_base = str(os.getenv("JIRA_BASE_URL", "")).strip()
    jira_token = str(os.getenv("JIRA_API_TOKEN", "")).strip()
    acceptance_path = Path(f"data/acceptance/{args.run_id}_acceptance.json")
    if not jira_base or not jira_token:
        print("skip: JIRA_BASE_URL/JIRA_API_TOKEN not set")
        return
    ...
    print(f"skip: jira publish simulated run_id={args.run_id} status={status}")
