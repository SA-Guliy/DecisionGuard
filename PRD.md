...
| Doctor primary goal alignment | Fixed: `ab_primary_goal` added to experiment header; goal alignment enforced at prompt level. Doctor reasoning score: 0.30 → 0.85 | `IMPLEMENTED` (2026-03-20) | Stable — included in ongoing eval runs |
...
| Live statistical evidence in agent reasoning | Doctor and Commander receive no ctrl/treatment delta statistics today | `P0 — planned` | `src/stat_engine.py` → inject `StatEvidenceBundle` into Doctor/Commander context; requires paired mode active |
| Reasoning confidence calibration | Reported as static value across all cases regardless of evidence strength | `P1 — planned` | `src/reasoning_confidence.py` — dynamic score based on layers present, p-value, sample size, historical similarity |
